...
const NAV = [
  ...
  { to: '/app/meetings', label: 'Meetings', icon: '🎙️' },
  { to: '/app/analytics', label: 'Analytics', icon: '📊' },
  { to: '/app/members', label: 'Members', icon: '👥' },
  { to: '/app/notifications', label: 'Notifications', icon: '🔔', badge: true },
    ...
    <div className="relative px-3">
      ...
      {open && (
        <div className="absolute left-3 right-3 top-full z-30 mt-1 rounded-lg border border-slate-200 bg-white p-1 shadow-lg" role="listbox">
          ...
          <button onClick={() => { setCreating(true); setOpen(false); }} className="mt-1 w-full rounded-md border-t border-slate-100 px-3 py-2 text-left text-sm text-brand-600 hover:bg-slate-50">+ New workspace</button>
        </div>
      )}
      ...
      <aside className={cn('fixed inset-y-0 left-0 z-40 flex w-64 flex-col border-r border-slate-200 bg-slate-50 py-4 transition-transform lg:static lg:translate-x-0', menu ? 'translate-x-0' : '-translate-x-full')}>
        ...
        <nav className="mt-4 flex-1 space-y-0.5 overflow-y-auto px-3" aria-label="Main">
          ...
          {user?.isPlatformAdmin && (
            <NavLink to="/app/admin" className={({ isActive }) => cn('flex items-center gap-3 rounded-lg px-3 py-2 text-sm font-medium', isActive ? 'bg-brand-100 text-brand-700' : 'text-slate-600 hover:bg-slate-100')}><span aria-hidden>🛡️</span>Platform admin</NavLink>
          )}
        </nav>
        <div className="mx-3 mt-2 flex items-center gap-2 border-t border-slate-200 pt-3">
          ...
          <button onClick={logout} title="Sign out" aria-label="Sign out" className="rounded-md p-1.5 text-slate-400 hover:bg-slate-200 hover:text-slate-700">⎋</button>
        </div>
      </aside>
      <div className="flex min-w-0 flex-1 flex-col">
        <VerifyEmailBanner />
        <header className="flex items-center gap-3 border-b border-slate-200 bg-white px-4 py-3 lg:hidden">
          <button onClick={() => setMenu(true)} aria-label="Open menu" className="rounded-md p-1.5 hover:bg-slate-100">☰</button>
