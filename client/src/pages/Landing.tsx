...
const FEATURES = [
  ['📋', 'Projects & Kanban', 'Plan work with boards, priorities, due dates and live drag-and-drop that syncs to everyone instantly.'],
  ['💬', 'Team chat', 'Channels, DMs, threads-lite replies, reactions, mentions and typing indicators — no extra app needed.'],
  ['📄', 'Document knowledge base', 'Upload PDFs, Word and Markdown. Ask questions and get answers with the exact page as the source.'],
  ['✨', 'AI Copilot', 'Summarise projects, find overdue work, decide what to do next, and turn requirements into tasks you approve.'],
  ['🎙️', 'Meeting assistant', 'Paste a transcript and get a summary, decisions, deadlines and reviewable action items.'],
  ['📊', 'Analytics & insights', 'Completion rate, workload, weekly throughput and explainable signals on where to look.'],
  ['🔐', 'Multi-tenant & secure', 'Workspace isolation enforced on the server, role-based access, audit logs and plan-based limits.'],
] as const;
          ...
          <div className="mt-10 grid gap-5 md:grid-cols-3">
            {PLANS.map(([name, price, items, hot]) => (
              <div key={name} className={`rounded-xl border bg-white p-6 ${hot ? 'border-brand-500 ring-2 ring-brand-200' : 'border-slate-200'}`}>
                ...
                <ul className="mt-4 space-y-2 text-sm text-slate-600">{items.map((i) => <li key={i}>✓ {i}</li>)}</ul>
              </div>
            ))}
          </div>
        </div>
